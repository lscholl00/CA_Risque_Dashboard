...
# Load data
inseedf_path = 'C:/Users/Source/Downloads/Hackathon_Open_Data/GASPARdata/insee_df.csv'
inseedf = pd.read_csv(inseedf_path)
...
def render_header():
    ...
    # Add title to col3
    with col3:
        st.title("Tableau de bord des Risques du Crédit Agricole")
        
# Cached function for tab1  
@st.cache_data
def tab1_cache():
    existing_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\predictions\coul_predictions.csv')
    existing_df['Code INSEE'] = existing_df['Code INSEE'].astype(str).str.strip()
    return existing_df

@st.cache_data
def tab2_cache():
    existing_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\predictions\rem_predictions.csv')
    existing_df['Code INSEE'] = existing_df['Code INSEE'].astype(str).str.strip()
    return existing_df

@st.cache_data
def tab3_cache():
    existing_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\predictions\sech_predictions.csv')
    existing_df['Code INSEE'] = existing_df['Code INSEE'].astype(str).str.strip()
    return existing_df
    
@st.cache_data
def load_historical():
    historical_risk_path = 'C:/Users/Source/Downloads/Hackathon_Open_Data/predictions/historical_risk.csv'
...
@st.cache_data
def inon_events():
    events_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\GASPARdata\inon_events.csv')
...
@st.cache_data
def sech_events():
    events_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\GASPARdata\sech_events.csv')
    events_df['dat_deb'] = pd.to_datetime(events_df['dat_deb'])
    events_df['dat_fin'] = pd.to_datetime(events_df['dat_fin'])
    events_df['duration'] = (events_df['dat_fin'] - events_df['dat_deb']).dt.days + 1
    return events_df
...
@st.cache_data
def cat_events():
    events_df = pd.read_csv(r'C:\Users\Source\Downloads\Hackathon_Open_Data\GASPARdata\cat_events.csv')
    events_df['dat_deb'] = pd.to_datetime(events_df['dat_deb'])
    return events_df
   ...
   # Merge the dataframes with inseedf to get 'Commune' and 'geometry'
    ...
    for _, row in gdf.iterrows():
        ...
        tooltip_text = (
            ...
            f"<b>Risque Moyen:</b> {row['average_risk']}</div>"
        )
    ...
    # Create colormap
    colormap = cm.LinearColormap(colors=['#FFFFB2','#FFD700', '#FFC300','#FFB000', '#FF8C00', '#FF7000', '#FF4500', '#FF2400', '#FF0000', '#CC0000','#8B0000'], vmin=0, vmax=1)
    colormap.caption = 'Risque Moyen (0 - 1) Low - High'
    colormap.add_to(m)
    ...
    <div style="position: fixed; 
         ...
         <b>Légende: Risque Moyen</b><br>
         0.0 - 0.1: <i style="background:#FFFFB2; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.1 - 0.2: <i style="background:#FFD700; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.2 - 0.3: <i style="background:#FFC300; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.3 - 0.4: <i style="background:#FFB000; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.4 - 0.5: <i style="background:#FF8C00; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.5 - 0.6: <i style="background:#FF7000; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.6 - 0.7: <i style="background:#FF4500; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.7 - 0.8: <i style="background:#FF2400; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.8 - 0.9: <i style="background:#FF0000; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         0.9 - 1.0: <i style="background:#CC0000; width: 18px; height: 18px; float: right; margin-right: 5px;"></i><br>
         1.0: <i style="background:#8B0000; width: 18px; height: 18px; float: right; margin-right: 5px;"></i>
    </div>
    '''
